...
def create_table_from_all_data(conn, data_list, table_name='unicom_data'):
    """根据所有数据文件创建表结构 - 收集threeDetailDate中所有字段的并集"""
    logger.info("分析所有文件，收集threeDetailDate完整字段信息...")
    cursor = conn.cursor()
    ...
    for item in data_list:
        ...
        if data and 'threeDetailDate' in data and data['threeDetailDate']:
            file_has_data = False
            
            for detail_item in data['threeDetailDate']:
                if detail_item:  # 确保detail_item不为空
                    file_has_data = True
                    total_records += 1
                    
                    # 1. 处理detailInfo内的字段
                    if 'detailInfo' in detail_item and detail_item['detailInfo']:
                        try:
                            detail_info = json.loads(detail_item['detailInfo'])
                            if detail_info and isinstance(detail_info, dict):
                                for key, value in detail_info.items():
                                    if key != 'accessWay':  # 忽略accessWay字段
                                        field_name = f"detailInfo_{key}"  # 添加前缀避免冲突
                                        _process_field(all_fields, field_name, value)
                        except (json.JSONDecodeError, TypeError) as e:
                            logger.warning(f"解析detailInfo失败 ({file_path}): {e}")
                    
                    # 2. 处理与detailInfo同级的其他字段
                    for key, value in detail_item.items():
                        if key not in ['detailInfo', 'broad', 'belongProvince']:  # 跳过这些字段
    ...
    if not all_fields:
        logger.error("没有找到有效的threeDetailDate数据用于创建表结构")
        raise ValueError("没有找到有效的threeDetailDate数据用于创建表结构")
    ...
    field_order = [
        'nameThird',
        'detailInfo_downSpeed', 
        'detailInfo_upSpeed',
        'detailInfo_netCountryFlow',
        'mainFee',
        'detailInfo_advanceDeposit',
        'serviceContent'
    ]
    ...
    for field_name in field_order:
        ...
        if field_name in field_dict:
            ...
            sample_str = str(field_info['sample'])[:50]
            logger.info(f"字段: {field_name} ({field_info['type']}) - 出现在 {field_info['record_count']} 条记录中 - 示例: {sample_str}...")
            
            # 为数值字段添加对应的数值列
            if field_name == 'mainFee':
                columns.append(f'"mainFee_numeric" INTEGER')
                logger.info(f"字段: mainFee_numeric (INTEGER) - 数值排序列")
            elif field_name == 'detailInfo_downSpeed':
                columns.append(f'"downSpeed_numeric" INTEGER')
                logger.info(f"字段: downSpeed_numeric (INTEGER) - 数值排序列")
            elif field_name == 'detailInfo_upSpeed':
                columns.append(f'"upSpeed_numeric" INTEGER')
                logger.info(f"字段: upSpeed_numeric (INTEGER) - 数值排序列")
            elif field_name == 'detailInfo_advanceDeposit':
                columns.append(f'"advanceDeposit_numeric" INTEGER')
                logger.info(f"字段: advanceDeposit_numeric (INTEGER) - 数值排序列")
            elif field_name == 'detailInfo_netCountryFlow':
                columns.append(f'"netCountryFlow_numeric" INTEGER')
                logger.info(f"字段: netCountryFlow_numeric (INTEGER) - 数值排序列")
...
def optimize_database(conn, table_name='unicom_data'):
...
def insert_data_to_db(conn, data_list, table_name='unicom_data'):
    """将数据插入数据库"""
    if not data_list:
    ...
    # 准备数据 - 处理threeDetailDate数组中的每个元素
    insert_data = []
    total_records = 0
    
    for item in data_list:
        three_detail_data = item['data'].get('threeDetailDate', [])
        
        if isinstance(three_detail_data, list):
            for detail_item in three_detail_data:
                if detail_item:  # 确保detail_item不为空
                    # 合并所有字段的数据
                    combined_data = {}
                    
                    # 1. 添加detailInfo内的字段（带前缀）
                    if 'detailInfo' in detail_item and detail_item['detailInfo']:
                        try:
                            detail_info = json.loads(detail_item['detailInfo'])
                            if detail_info and isinstance(detail_info, dict):
                                for key, value in detail_info.items():
                                    if key != 'accessWay':  # 忽略accessWay字段
                                        field_name = f"detailInfo_{key}"
                                        combined_data[field_name] = value
                        except (json.JSONDecodeError, TypeError) as e:
                            logger.warning(f"跳过无效的detailInfo JSON ({item['file_path']}): {e}")
                    
                    # 2. 添加与detailInfo同级的其他字段
                    for key, value in detail_item.items():
                        if key not in ['detailInfo', 'broad', 'belongProvince']:  # 跳过这些字段
                    ...
                    for field in db_columns:
                        if field.endswith('_numeric'):
                            # 处理数值列
                            if field == 'mainFee_numeric':
                                source_value = combined_data.get('mainFee', '')
                            elif field == 'downSpeed_numeric':
                                source_value = combined_data.get('detailInfo_downSpeed', '')
                            elif field == 'upSpeed_numeric':
                                source_value = combined_data.get('detailInfo_upSpeed', '')
                            elif field == 'advanceDeposit_numeric':
                                source_value = combined_data.get('detailInfo_advanceDeposit', '')
                            elif field == 'netCountryFlow_numeric':
                                source_value = combined_data.get('detailInfo_netCountryFlow', '')
                            else:
                                source_value = ''
                            ...
                            import re
                            if source_value:
                                numbers = re.findall(r'\d+', str(source_value))
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='处理联通数据并导入PostgreSQL数据库')
    parser.add_argument('folder_path', help='包含response.dump文件的文件夹路径')
    parser.add_argument('--table-name', default='unicom_data', help='数据库表名 (默认: unicom_data)')
